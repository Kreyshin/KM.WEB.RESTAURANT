...
<template>
  <div
    class="rs-campo flex h-[var(--km-toque,2.5rem)] w-full items-stretch overflow-hidden rounded-control border bg-panel text-[length:var(--km-texto,0.875rem)] transition-colors duration-200"
    :class="[
      invalido ? 'border-vino' : 'border-linea focus-within:border-verde',
  ...
  >
    ...
    <input
      ...
      :aria-valuemin="min"
      :aria-valuemax="max"
      class="rs-sin-anillo min-w-0 flex-1 bg-transparent px-3 text-tinta tabular-nums outline-none placeholder:text-tenue/70"
